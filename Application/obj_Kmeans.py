...
def res_fit(path,filenames, labels):
    files = [file.split('\\')[-1] for file in filenames]
    for i in range(len(labels)):
        if not os.path.exists(path + '/result/'+ str(labels[i])):
            os.makedirs(path + '/result/'+ str(labels[i]))
        shutil.copy(filenames[i],path + '/result/'+ str(labels[i])+"/"+ files[i])
...
def main():
    path = r'C:/Users/zkteco/Desktop/test/cutout/Liqu'
    path_filenames = get_file_name(path)

    labels, cluster_centers = knn_detect(path_filenames[:10000],4)

    res_dict = res_fit(path,path_filenames[:10000], labels)
    save(path, 'knn_res.txt', res_dict)
